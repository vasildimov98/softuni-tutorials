...
export function removeNotification(e) {
    e.target.style.display = 'none';
}

export function getAllUserDestination(destionations) {
    const userDestionation = [];
    for (const dest of destionations) {
        if (dest._ownerId == getUserLocalId())
            userDestionation.push(dest);
    }
    return userDestionation;
}